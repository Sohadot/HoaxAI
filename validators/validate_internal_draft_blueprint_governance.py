...
def validate_publisher_and_gates() -> bool:
    ...
    if status not in (
        ...
        error(
            ...
            f"blocked_until_first_internal_draft_blueprint_pack, blocked_until_first_internal_draft_pack, blocked_until_internal_draft_review_and_refinement, or blocked_until_public_route_readiness_gate, got {status}"
        )
        ok = False
...
def validate_route_sitemap_public_safety() -> bool:
    ...
    for candidate in registry.get("candidates", []):
        slug = candidate.get("proposed_slug", "").lower()
